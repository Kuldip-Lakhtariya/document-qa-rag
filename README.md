...
- **No relevance rejection — the retriever always returns its "best" chunks, even for a completely off-topic question.** `VectorDB.search()` returns the `top_k` *nearest* chunks by vector distance, with no threshold for "none of these are actually relevant." Ask the app something totally unrelated to the uploaded document (e.g. a geography question against a company handbook), and it still retrieves 3 chunks — whichever happen to be least-dissimilar — and the LLM will attempt an answer grounded in them anyway, potentially confidently wrong. This is the same structural shape as a closed-set image classifier having no "not a leaf" option: a system built to always return the best of its available candidates, with no way to say "none of these apply." More/better chunking doesn't fix this, since it's a property of similarity search returning a ranked list rather than a relevance gate. A real fix needs a distance-threshold cutoff (reject if the nearest chunk is still too far away) or a lightweight "is this question answerable from this document" classifier before retrieval runs. Documented here, not yet implemented.
